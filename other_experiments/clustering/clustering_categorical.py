...
def objective(
    ...
    cat_dissim_name = trial.suggest_categorical("cat_dissim", ["matching", "ng"])
    cat_dissim = DISSIM_NAME_CLASS_MAP[cat_dissim_name]
    ...
    df_train = df_train.loc[:, categorical_columns + boolean_columns]
    model.fit(df_train)
...
if "__main__" == __name__:
    ...
    study.optimize(
        lambda trial: objective(
            trial, df_train[: int(1e5)], df_val, categorical_columns, boolean_columns
        ),
        n_trials=N_TRIALS,
